...
const DEFAULT_TEXT=GROUPS.map(g=>`## ${g.title}\n${g.items.map(([n,label])=>`${String(n).padStart(2,'0')} ${label}`).join('\n')}`).join('\n');
const PALETTE=GROUPS.map(g=>g.color);
const defaults={version:2,visible:false,title:DEFAULT_TITLE,text:DEFAULT_TEXT,x:18,y:18,width:410,fontSize:10,bg:'#fffdf3',color:'#263845',border:'#60746f',borderWidth:1.5};
...
function contentRows(){let section=-1,color=PALETTE[0];const rows=[];String(state.text||'').replace(/\r\n?/g,'\n').split('\n').forEach(raw=>{const line=raw.trim();if(!line)return;const header=line.match(/^##\s*(.+)$/u);if(header){section++;color=PALETTE[section%PALETTE.length];rows.push({type:'header',text:header[1],color});return}const item=line.match(/^(\d{1,3})\s+(.+)$/u);if(item){rows.push({type:'item',number:item[1],text:item[2],color});return}rows.push({type:'text',text:line,color})});return rows}
function layout(){const pad=12,titleSize=state.fontSize+4,rowH=state.fontSize*1.46,sectionH=state.fontSize*1.52,rows=contentRows(),height=pad+(state.title.trim()?titleSize*1.25+7:0)+rows.reduce((h,row)=>h+(row.type==='header'?sectionH:rowH),0)+pad;return{pad,titleSize,rowH,sectionH,rows,height:clamp(height,80,1400)}}
