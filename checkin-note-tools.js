...
const defaults={version:3,visible:false,title:DEFAULT_TITLE,text:DEFAULT_TEXT,x:18,y:18,width:410,fontSize:10,bg:'#fffdf3',color:'#263845',border:'#60746f',borderWidth:1.5};
...
function load(){try{const raw=JSON.parse(localStorage.getItem(STORE)||'{}');Object.assign(state,normalize(raw));if((Number(raw.version)||0)<3&&(state.text===LEGACY_GROUPED_TEXT||isComplete38(state.text))){state.text=sortedText(state.text);localStorage.setItem(STORE,JSON.stringify(snapshot()))}}catch{Object.assign(state,defaults)}}
