...
const defaults={version:1,visible:false,x:18,y:18,width:410,fontSize:10,bg:'#fffdf3',color:'#263845',border:'#60746f',borderWidth:1.5};
...
function normalize(d={}){return{version:1,visible:d.visible===true,x:Number.isFinite(Number(d.x))?Number(d.x):defaults.x,y:Number.isFinite(Number(d.y))?Number(d.y):defaults.y,width:clamp(Number(d.width)||defaults.width,300,620),fontSize:clamp(Number(d.fontSize)||defaults.fontSize,8,16),bg:color(d.bg,defaults.bg),color:color(d.color,defaults.color),border:color(d.border,defaults.border),borderWidth:clamp(Number(d.borderWidth)||defaults.borderWidth,.5,6)}}
function load(){try{Object.assign(state,normalize(JSON.parse(localStorage.getItem(STORE)||'{}')))}catch{Object.assign(state,defaults)}}
...
function layout(){const pad=12,titleSize=state.fontSize+4,rowH=state.fontSize*1.46,sectionH=state.fontSize*1.52,total=GROUPS.reduce((n,g)=>n+g.items.length,0),height=pad+titleSize*1.25+7+GROUPS.length*sectionH+total*rowH+pad;return{pad,titleSize,rowH,sectionH,height}}
...
function render(){
 ...
 let y=L.pad+L.titleSize;
 appendText(g,{x:state.width/2,y,'text-anchor':'middle','font-family':FONT,'font-size':L.titleSize,'font-weight':900,fill:state.color},'38 ĐIỂM CHECK-IN XUYÊN VIỆT');y+=7;
 for(const group of GROUPS){
   const top=y+state.fontSize*.12;g.appendChild(el('rect',{x:L.pad,y:top,width:state.width-L.pad*2,height:state.fontSize*1.28,rx:4,ry:4,fill:group.color,'fill-opacity':.14}));
   y+=state.fontSize*1.08;appendText(g,{x:L.pad+6,y,'font-family':FONT,'font-size':state.fontSize*.92,'font-weight':900,fill:group.color},group.title);y+=L.sectionH-state.fontSize*1.08;
   for(const [num,label] of group.items){
     y+=L.rowH;const r=state.fontSize*.55,cx=L.pad+r,cy=y-state.fontSize*.34;
     g.appendChild(el('circle',{cx,cy,r,fill:group.color}));appendText(g,{x:cx,y:cy+state.fontSize*.2,'text-anchor':'middle','font-family':FONT,'font-size':state.fontSize*.54,'font-weight':900,fill:'#ffffff'},String(num).padStart(2,'0'));
     const parts=label.split(' – '),t=el('text',{x:L.pad+state.fontSize*1.48,y,'font-family':FONT,'font-size':state.fontSize,fill:state.color});
     const a=el('tspan',{'font-weight':800});a.textContent=parts[0];t.appendChild(a);if(parts.length>1){const b=el('tspan',{'font-weight':500});b.textContent=' – '+parts.slice(1).join(' – ');t.appendChild(b)}g.appendChild(t)
   }
 }
...
function injectUI(){
 ...
 [['checkinNoteWidth','width'],['checkinNoteFontSize','fontSize'],['checkinNoteBg','bg'],['checkinNoteColor','color'],['checkinNoteBorder','border'],['checkinNoteBorderWidth','borderWidth'],['checkinNoteX','x'],['checkinNoteY','y']].forEach(([id,key])=>$(id).addEventListener('input',e=>setField(key,e.target.value)));syncUI()
}
