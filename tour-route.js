...
const STORE='vn-xuyen-viet-route-standard38-v8';
const GEO={minLon:101.7,maxLon:110.7,minLat:7.4,maxLat:23.7};
const PLOT={x:350,y:18,w:700,h:954};

// Cung đường dựng lại từ chương trình chuẩn 38 ngày - 37 đêm.
...
function defaults(){return{version:8,points:BASE.map(p=>({...p})),bends:ROUTE.slice(0,-1).map((_,i)=>Number(ROUTE[i+1]?.bend)||0),color:'#d71945',seaColor:'#1677d2',width:2.5,show:true,showNumbers:true,showArrows:true}}
function load(){const fresh=defaults();try{const d=JSON.parse(localStorage.getItem(STORE)||'null');if(d&&d.version===8&&Array.isArray(d.points)&&d.points.length===ROUTE.length){data={...fresh,...d};data.points=d.points.map((p,i)=>({x:Number(p?.x)||fresh.points[i].x,y:Number(p?.y)||fresh.points[i].y}));data.bends=ROUTE.slice(0,-1).map((_,i)=>Number(d.bends?.[i])||0);return}}catch{}data=fresh}
