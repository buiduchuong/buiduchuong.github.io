...
const STORE='vn-xuyen-viet-route-overview-v4';
const GEO={minLon:101.7,maxLon:110.7,minLat:7.4,maxLat:23.7};
const PLOT={x:350,y:18,w:700,h:954};
const ROUTE=[
 {name:'Hà Nội',lat:21.024167,lon:105.857778,num:1,start:true,offset:{x:-10,y:-8}},
 {name:'Quảng Bình',lat:17.4580,lon:106.6359,num:2,bend:12},
 {name:'Huế',lat:16.456111,lon:107.576389,num:3,bend:12},
 {name:'Đà Nẵng',lat:16.067780,lon:108.220830,num:4,bend:10},
 {name:'Quy Nhơn',lat:13.769583,lon:109.231389,num:5,bend:8,offset:{x:-8,y:-6}},
 {name:'Măng Đen',lat:14.600278,lon:108.290833,num:6,bend:-8},
 {name:'Pleiku',lat:13.971519,lon:108.014673,num:7,bend:-7},
 {name:'Buôn Ma Thuột',lat:12.667470,lon:108.037750,num:8,bend:-5},
 {name:'Đà Lạt',lat:11.940000,lon:108.437500,num:9,bend:6},
 {name:'Tà Đùng',lat:11.8350,lon:107.9980,num:10,bend:5},
 {name:'TP.HCM',lat:10.823020,lon:106.629650,num:11,bend:8,offset:{x:-10,y:-8}},
 {name:'Tiền Giang · Bến Tre',lat:10.3000,lon:106.3700,num:12,bend:5},
 {name:'Cần Thơ',lat:10.037220,lon:105.788330,num:13,bend:5},
 {name:'Cà Mau',lat:8.7590,lon:105.0010,num:14,bend:-5},
 {name:'Hà Tiên · Rạch Giá',lat:10.2000,lon:104.8000,num:15,bend:-7},
 {name:'Phú Quốc',lat:10.2167,lon:103.9670,num:16,sea:true,bend:-8,offset:{x:-4,y:-6}},
 {name:'Hà Tiên',lat:10.3833,lon:104.4875,hidden:true,sea:true,bend:8},
 {name:'Châu Đốc',lat:10.7000,lon:105.116667,num:17,bend:5},
 {name:'Đồng Tháp',lat:10.4500,lon:105.6500,num:18,bend:5},
 {name:'TP.HCM',lat:10.823020,lon:106.629650,num:19,bend:10,offset:{x:10,y:-8}},
 {name:'Tây Ninh',lat:11.364053,lon:106.180054,num:20,bend:-9},
 {name:'Vũng Tàu',lat:10.404167,lon:107.141667,num:21,bend:12},
 {name:'Mũi Né',lat:10.981999,lon:108.251183,num:22,bend:8},
 {name:'Ninh Chữ · Vĩnh Hy',lat:11.6800,lon:109.1200,num:23,bend:5},
 {name:'Nha Trang',lat:12.2450,lon:109.191667,num:24,bend:5},
 {name:'Phú Yên',lat:13.086872,lon:109.308589,num:25,bend:6},
 {name:'Quy Nhơn',lat:13.769583,lon:109.231389,num:26,bend:-12,offset:{x:8,y:7}},
 {name:'Quảng Ngãi',lat:15.1200,lon:108.8050,num:27,bend:-7},
 {name:'Hội An',lat:15.877222,lon:108.329167,num:28,bend:-6},
 {name:'Đà Nẵng',lat:16.067780,lon:108.220830,num:29,bend:-12,offset:{x:8,y:7}},
 {name:'Huế',lat:16.456111,lon:107.576389,num:30,bend:-15,offset:{x:8,y:7}},
 {name:'Quảng Bình',lat:17.4580,lon:106.6359,num:31,bend:-16,offset:{x:8,y:7}},
 {name:'Nghệ An',lat:18.671430,lon:105.558720,num:32,bend:-8},
 {name:'Ninh Bình',lat:20.255694,lon:105.915611,num:33,bend:-7},
 {name:'Hà Nội',lat:21.024167,lon:105.857778,num:34,finish:true,bend:-12,offset:{x:10,y:8}}
];
...
function defaults(){return{version:4,points:BASE.map(p=>({...p})),bends:ROUTE.slice(0,-1).map((p,i)=>Number(ROUTE[i+1]?.bend)||0),color:'#d71945',seaColor:'#1677d2',width:2.6,show:true,showNumbers:true,showArrows:true}}
function load(){const fresh=defaults();try{const d=JSON.parse(localStorage.getItem(STORE)||'null');if(d&&d.version===4&&Array.isArray(d.points)&&d.points.length===ROUTE.length){data={...fresh,...d};data.points=d.points.map((p,i)=>({x:Number(p?.x)||fresh.points[i].x,y:Number(p?.y)||fresh.points[i].y}));data.bends=ROUTE.slice(0,-1).map((_,i)=>Number(d.bends?.[i])||0);return}}catch{}data=fresh}
...
function addNumberMarker(g,r,active){g.appendChild(el('circle',{r:active?8.9:7.8,fill:'#fff','fill-opacity':'.98',stroke:r.finish?'#16803a':data.color,'stroke-width':active?2.7:1.9,'vector-effect':'non-scaling-stroke'}));if(data.showNumbers){const t=el('text',{x:0,y:2.35,'text-anchor':'middle','font-family':'Roboto,Arial,sans-serif','font-size':r.num>=10?5.7:6.5,'font-weight':900,fill:r.finish?'#16803a':data.color,'pointer-events':'none'});t.textContent=String(r.num);g.appendChild(t)}else g.appendChild(el('circle',{r:2.5,fill:r.finish?'#16803a':data.color,'pointer-events':'none'}));if(r.start){const star=el('text',{x:-8.5,y:-6.5,'text-anchor':'middle','font-family':'Arial,sans-serif','font-size':10.5,'font-weight':900,fill:'#f4b400','pointer-events':'none'});star.textContent='★';g.appendChild(star)}if(r.finish){const check=el('text',{x:8.5,y:-6.5,'text-anchor':'middle','font-family':'Roboto,Arial,sans-serif','font-size':8.5,'font-weight':900,fill:'#16803a','pointer-events':'none'});check.textContent='✓';g.appendChild(check)}}
function render(){if(!layer)return;layer.innerHTML='';if(!data.show)return;ensureDefs();const segs=el('g',{id:'tourOverviewSegments'}),marks=el('g',{id:'tourOverviewMarkers'});for(let i=0;i<data.points.length-1;i++){const s=data.points[i],e=data.points[i+1],c=controlPoint(s,e,data.bends[i]||0),sea=isSea(i),stroke=sea?data.seaColor:data.color;const p=el('path',{d:`M${s.x},${s.y} Q${c.x},${c.y} ${e.x},${e.y}`,fill:'none',stroke,'stroke-width':selected===i?Number(data.width)+1.15:data.width,'stroke-linecap':'round','stroke-linejoin':'round','vector-effect':'non-scaling-stroke',opacity:selected===i?'1':'.93',cursor:'pointer','data-tour-segment':i});if(sea)p.setAttribute('stroke-dasharray','7 5');if(shouldArrow(i))p.setAttribute('marker-end',sea?'url(#tourOverviewSeaArrow)':'url(#tourOverviewArrow)');addTitle(p,`${ROUTE[i].num||''}. ${ROUTE[i].name} → ${ROUTE[i+1].num||''}. ${ROUTE[i+1].name}${sea?' · tàu/đường biển':''}`);p.addEventListener('click',ev=>{ev.stopPropagation();selected=i;syncSegment();render()});segs.appendChild(p)}ROUTE.forEach((r,i)=>{if(r.hidden)return;const p=data.points[i],o=off(i),g=el('g',{transform:`translate(${p.x+o.x} ${p.y+o.y})`,'data-tour-point':i,cursor:'grab'}),active=selectedPoint===i;addNumberMarker(g,r,active);addTitle(g,r.start?`★ 1. Khởi hành: ${r.name}`:r.finish?`✓ ${r.num}. Kết thúc: ${r.name}`:`${r.num}. ${r.name}`);g.addEventListener('pointerdown',startPointDrag);g.addEventListener('click',ev=>{ev.stopPropagation();selectedPoint=i;syncPoint();render()});marks.appendChild(g)});layer.append(segs,marks)}
