...
function save(flush=false){try{localStorage.setItem(STORE,JSON.stringify({version:2,flags:state.flags,show:state.show,dragEnabled:state.dragEnabled,updatedAt:Date.now()}))}catch(e){console.warn('Không lưu được cờ',e);setImageStatus('Dung lượng lưu cờ đã đầy. Hãy dùng ảnh nhỏ hơn.','error')}if(flush&&window.__VN_PERSIST?.flush)window.__VN_PERSIST.flush();else scheduleFlush()}
...
function imageBox(f){const dir=f.direction==='left'?-1:1,w=f.imageW,h=f.imageH,poleH=Math.max(h*1.75,64),x=dir<0?-w:0,y=-poleH;return{dir,w,h,poleH,x,y}}
function drawTriangleFlag(g,f){const s=f.size,poleH=s*1.75,flagH=s*.68,flagW=s,dir=f.direction==='left'?-1:1,minX=dir<0?-flagW-s*.25:-s*.25;g.appendChild(el('rect',{x:minX,y:-poleH-s*.12,width:flagW+s*.5,height:poleH+s*.38,class:'flag-hit'}));g.appendChild(el('line',{x1:0,y1:4,x2:0,y2:-poleH,class:'flag-pole',stroke:f.poleColor,'stroke-width':Math.max(1.7,s*.065)}));g.appendChild(el('polygon',{points:`0,${-poleH} ${dir*flagW},${-poleH+flagH*.48} 0,${-poleH+flagH}`,class:'flag-shape',fill:f.flagColor}));g.appendChild(el('circle',{cx:0,cy:4,r:Math.max(2.2,s*.095),class:'flag-base',fill:f.poleColor}))}
...
function addTriangleFlag(){const[x,y]=visibleCenter(),f=normalizeFlag({id:uid(),x,y,kind:'triangle',size:34,flagColor:'#e51d49',poleColor:'#333333'});state.flags.push(f);state.selected=f.id;save();render()}
function addImageFlag(){const[x,y]=visibleCenter(),f=normalizeFlag({id:uid(),x,y,kind:'image',imageW:82,imageH:52,poleColor:'#333333'});state.flags.push(f);state.selected=f.id;save();render();setImageStatus('Chọn ảnh từ máy hoặc dán URL ảnh.','')}
...
function setImageDimension(axis,v){const f=selectedFlag();if(!f||f.kind!=='image')return;const value=axis==='w'?clamp(Number(v)||82,30,280):clamp(Number(v)||52,20,220);if(axis==='w'){f.imageW=value;if(f.lockRatio)f.imageH=clamp(value/(f.imageRatio||1.5769),20,220)}else{f.imageH=value;if(f.lockRatio)f.imageW=clamp(value*(f.imageRatio||1.5769),30,280)}save();render()}
...
$('flagSelect')?.addEventListener('change',e=>select(e.target.value));$('flagType')?.addEventListener('change',e=>{const f=selectedFlag();if(!f)return;f.kind=e.target.value==='image'?'image':'triangle';save();render()});$('flagSize')?.addEventListener('input',e=>setSize(e.target.value));$('flagSizeRange')?.addEventListener('input',e=>setSize(e.target.value));$('flagColor')?.addEventListener('input',e=>{const f=selectedFlag();if(!f)return;f.flagColor=e.target.value;save();render()});$('flagPoleColor')?.addEventListener('input',e=>{const f=selectedFlag();if(!f)return;f.poleColor=e.target.value;save();render()});$('flagDirection')?.addEventListener('change',e=>{const f=selectedFlag();if(!f)return;f.direction=e.target.value==='left'?'left':'right';save();render()});$('flagX')?.addEventListener('input',e=>setPos('x',e.target.value));$('flagY')?.addEventListener('input',e=>setPos('y',e.target.value));$('showFlags')?.addEventListener('change',e=>{state.show=e.target.checked;save();render()});$('dragFlags')?.addEventListener('change',e=>{state.dragEnabled=e.target.checked;save();render()});
$('flagImageW')?.addEventListener('input',e=>setImageDimension('w',e.target.value));$('flagImageWRange')?.addEventListener('input',e=>setImageDimension('w',e.target.value));$('flagImageH')?.addEventListener('input',e=>setImageDimension('h',e.target.value));$('flagImageHRange')?.addEventListener('input',e=>setImageDimension('h',e.target.value));$('flagLockRatio')?.addEventListener('change',e=>{const f=selectedFlag();if(!f)return;f.lockRatio=e.target.checked;f.imageRatio=f.imageW/f.imageH;save();render()});$('flagImageFit')?.addEventListener('change',e=>{const f=selectedFlag();if(!f)return;f.imageFit=e.target.value==='contain'?'contain':'cover';save();render()});$('flagBorderColor')?.addEventListener('input',e=>{const f=selectedFlag();if(!f)return;f.borderColor=e.target.value;save();render()});$('flagBorderWidth')?.addEventListener('input',e=>{const f=selectedFlag();if(!f)return;f.borderWidth=clamp(Number(e.target.value)||0,0,8);save();render()});$('flagRadius')?.addEventListener('input',e=>{const f=selectedFlag();if(!f)return;f.radius=clamp(Number(e.target.value)||0,0,30);save();render()});
