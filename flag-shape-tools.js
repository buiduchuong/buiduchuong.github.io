...
function setField(key,v){
  ...
  }else if(key==='name')f.name=String(v||'').replace(/\s+/g,' ').trimStart().slice(0,80);
...
function imageShapeName(f){const i=state.items.indexOf(f)+1,name=f?.name||f?.tourName||BUILTIN_NAMES[f?.id]||'Shape ảnh';return `${i}. ${name} · ${f?.side==='left'?'trái':'phải'}`}
...
function syncUI(){
  ...
  if(s){const cur=state.selected;s.innerHTML=state.items.length?'':'<option value="">Chưa có shape</option>';state.items.forEach((x,i)=>{const o=document.createElement('option');o.value=x.id;const title=x.name||typeLabel(x.type);o.textContent=`${i+1}. ${title} · ${x.side==='left'?'trái':'phải'}`;s.appendChild(o)});if(cur)s.value=cur}
