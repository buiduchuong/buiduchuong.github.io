...
function normalize(f,i=0){
  ...
  return{id:String(f?.id||uid()),x:Number.isFinite(Number(f?.x))?Number(f.x):700+i*12,y:Number.isFinite(Number(f?.y))?Number(f.y):500+i*8,type,side:f?.side==='left'?'left':'right',offset:clamp(Number(f?.offset)||34,10,320),w,h,fill:color(f?.fill,'#f0c95e'),border:color(f?.border,'#554b43'),borderWidth:clamp(Number(f?.borderWidth)||1.8,.5,8),connectorColor:color(f?.connectorColor,'#554b43'),connectorWidth:clamp(Number(f?.connectorWidth)||1.7,.5,8),radius:clamp(Number(f?.radius)||4,0,30),imageSrc:String(f?.imageSrc||''),imageUrl:String(f?.imageUrl||''),imageRatio,lockRatio:f?.lockRatio!==false,imageFit:f?.imageFit==='cover'?'cover':'contain'}
}
...
function save(flush=false){try{localStorage.setItem(STORE,JSON.stringify({version:2,items:state.items,show:state.show,dragEnabled:state.dragEnabled,updatedAt:Date.now()}))}catch(e){console.warn('Không lưu được shape cờ',e);setImageStatus('Dung lượng lưu shape ảnh đã đầy. Hãy dùng ảnh nhỏ hơn.','error')}if(flush&&window.__VN_PERSIST?.flush)window.__VN_PERSIST.flush();else{clearTimeout(state.saveTimer);state.saveTimer=setTimeout(()=>window.__VN_PERSIST?.flush?.(),450)}}
...
function appendBody(g,f,b){
  if(f.type==='image')appendImageBody(g,f,b);
    ...
    let body;
    if(f.type==='circle')body=el('ellipse',{cx:b.x+b.w/2,cy:0,rx:b.w/2,ry:b.h/2,fill:f.fill,stroke:f.border,'stroke-width':f.borderWidth,class:'shape-body'});
    else if(f.type==='triangle'){const pts=b.side>0?`${b.x},${b.y} ${b.x+b.w},0 ${b.x},${b.y+b.h}`:`${b.x+b.w},${b.y} ${b.x},0 ${b.x+b.w},${b.y+b.h}`;body=el('polygon',{points:pts,fill:f.fill,stroke:f.border,'stroke-width':f.borderWidth,'stroke-linejoin':'round',class:'shape-body'})}
...
function syncUI(){
  ...
  if(!f)return;
  const vals={shapeFlagType:f.type,shapeFlagSide:f.side,shapeFlagOffset:Math.round(f.offset),shapeFlagRadius:f.radius,shapeFlagW:Math.round(f.w),shapeFlagH:Math.round(f.h),shapeFlagFill:f.fill,shapeFlagBorder:f.border,shapeFlagBorderWidth:f.borderWidth,shapeFlagConnectorColor:f.connectorColor,shapeFlagConnectorWidth:f.connectorWidth,shapeFlagX:Math.round(f.x),shapeFlagY:Math.round(f.y)};Object.entries(vals).forEach(([id,v])=>{if($(id))$(id).value=v});
