...
let tries=0;
async function boot(){tries++;const installed=await install(),ui=injectToggle(),observed=observeLayer();applyVisibility();if(installed&&ui&&observed)return;if(tries<12)setTimeout(boot,900);else console.warn('Shape tool hoặc ảnh nội bộ chưa sẵn sàng; không reload trang.')}
...
(()=>{
 ...
 const s=document.createElement('script');s.src='png-export-fix.js?v=2';s.dataset.vnPngExportFix='1';s.onerror=()=>console.warn('Không tải được bộ sửa xuất PNG 4K/8K');document.body.appendChild(s);
})();
